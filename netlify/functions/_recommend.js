...
const SERVICE_LIBRARY = {
  ...
  contracts: { label: 'Government Contracts', icon: '🏛', href: 'https://capgen.aproposgroupllc.com', blurb: 'Federal contract intelligence via CapGen — plus Nevada & California state matching.' },
  capability: { label: 'Capability Statement', icon: '🧾', href: 'https://capgen.aproposgroupllc.com', blurb: 'Build the profile government buyers and partners expect.' },
...
function recommend(input) {
  ...
  const add = (key, reason) => {
    if (seen.has(key) || !SERVICE_LIBRARY[key]) return;
    seen.add(key);
    rec.push({ key, reason });
  };
  ...
  if (wantsCustomers) {
    add('marketing', 'Because your business needs stronger customer acquisition support.');
    add('customers', 'Because you need a repeatable way to land your first or next customers.');
  }
  if (wantsFunding) add('funding', 'Because you indicated funding is a current priority.');
  if (wantsContracts) {
    add('contracts', registrationExists ? 'Because your existing registration can be used to open your contract intelligence dashboard.' : 'Because government contract readiness is part of your growth path.');
    add('capability', 'Because government buyers will expect a strong capability statement.');
    add('proposal', 'Because winning contracts means responding with organized proposals.');
  }
  if (noBasics && !registrationExists) {
    add('formation', "Because you're still standing up the basics of your business.");
    add('documents', "Because you'll need core business documents in place early.");
  }
